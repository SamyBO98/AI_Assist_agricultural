...
def predire_rendement(
    ...
    if azote < 120:
        conseils.append(
            f"Azote faible ({azote:.0f} kg/ha) envisager un apport complémentaire"
        )
    elif azote > 220:
        conseils.append(
            f"Azote excessif ({azote:.0f} kg/ha) risque de verse et pollution nitrique"
        )
    if ph_sol < 6.0:
        conseils.append(
            f"pH acide ({ph_sol:.1f}) chaulage recommandé (objectif 6.5–7.0)"
        )
    if matiere_org < 1.5:
    ...
    if not conseils:
        conseils.append(
            "Paramètres globalement favorables maintenir les pratiques actuelles"
        )
