...
def analyser_vache(
    ...
    if ccs > 400:
        alertes.append(
            f"CCS très élevé ({ccs:.0f} k/mL) suspicion mammite, prélèvement urgent"
        )
    elif ccs > 200:
        alertes.append(
            f"CCS élevé ({ccs:.0f} k/mL) surveiller l'évolution, hygiène traite"
        )

    if temperature_v > 39.5:
        alertes.append(
            f"Hyperthermie ({temperature_v:.1f}°C) fièvre possible, appeler vétérinaire"
        )

    if production < 12:
        alertes.append(
            f"Production faible ({production:.1f} L/j) vérifier alimentation et confort"
        )
