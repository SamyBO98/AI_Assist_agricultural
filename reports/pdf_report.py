...
def hr(color=GRIS_BORD):
    return HRFlowable(
        width="100%", thickness=1, color=color, spaceAfter=8, spaceBefore=4
    )
...
def badge_risque(risque, label, couleur):
    ...
    bg = (
        VERT_CLAIR
        if label == "Faible"
        else (ORANGE_CLAIR if label in ("Modéré", "Élevé") else ROUGE_CLAIR)
    )
    t = Table(
        [[f"Indice de risque agronomique : {risque}/100 — {label}"]],
        colWidths=[170 * mm],
    )
    t.setStyle(
        TableStyle(
...
def calcul_priorite(score_sante, ccs, temperature_v, bcs, production):
    ...
    """
    critique = ccs > 400 or temperature_v > 39.5 or bcs < 2.0 or production < 10
    vigilance = ccs > 200 or temperature_v > 38.8 or bcs < 2.5 or score_sante < 40
...
def badge_priorite(label, couleur, bg):
    t = Table([[f"Priorité d'intervention : {label}"]], colWidths=[170 * mm])
    t.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, -1), bg),
...
def fig_importance(importances):
    ...
    colors_bar = [
        "#2e7d32" if i == np.argmax(importances) else "#81c784"
...
def export_pdf_culture(
    ...
    **_,
    ...
    # En-tête
    entete(story, styles, "Rapport d'analyse parcelle", f"Généré le {date_str}")
    ...
    t.setStyle(
        TableStyle(
            [
                ...
                ("BOTTOMPADDING", (0, 0), (-1, -1), 5),
                ("LEFTPADDING", (0, 0), (-1, -1), 8),
                ("TEXTCOLOR", (1, 3), (1, 3), VERT if ecart >= 0 else ROUGE),
            ]
        )
    )
    story.append(t)
...
def export_pdf_vache(
    ...
    **_,
    ...
    # En-tête
    entete(story, styles, "Rapport d'analyse animal", f"Généré le {date_str}")
    ...
    if anomalie_reelle:
        ...
        story.append(
            Paragraph(
                "Aucune anomalie détectée : animal en bonne santé apparente.",
                styles["Conseil"],
            )
        )
