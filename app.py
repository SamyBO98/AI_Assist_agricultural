...
def valider_champs(valeurs, bornes):
    ...
    for label, valeur in valeurs.items():
        ...
        if valeur is None:
            ...
            erreurs.append(
                li(f"{label} : valeur hors limites physiques ({plage}) — {justif}")
            )
    return erreurs
...
def pipeline_wrapper(
    temperature, pluviometrie, azote, ph_sol, matiere_org, densite_semis, type_sol
):
...
def pipeline_vache_wrapper(
    production, taux_tb, taux_tp, temperature_v, ccs, bcs, age_mois, lactation_j
):
...
def pipeline_feuille_wrapper(image):
    ...
    if not resultat.get("success"):
        return (
            None,
            "",
            f"<p style='color:red'>{resultat.get('error', 'Erreur inconnue')}</p>",
        )
...
with gr.Blocks() as interface:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Culture"):
            ...
            with gr.Row():
                ...
                with gr.Column(scale=2):
                    ...
                    btn_pdf1 = gr.Button(
                        "Exporter en PDF", variant="secondary", visible=False
                    )
            ...
            btn_pdf1.click(fn=export_culture_pdf, inputs=[], outputs=[pdf1])
        ...
        with gr.Tab("Vache"):
            ...
            btn_pdf_v.click(fn=export_vache_pdf, inputs=[], outputs=[pdf_v])
